...
module.exports = {
  "create": async ( req, res ) => {
    ...
    const { body } = req.body,
      packageInfo = await Package.findOne( { "_id": req.body._package } ),
      newAgency = new Agency( { ...body, "expire": { "leftTime": packageInfo.total } } );
  ...
  "index": async ( req, res ) => {
    ...
    if ( req.query._id ) {
      dataResponse = await Agency.findOne( { "_id": req.query._id } ).populate( { "path": "_account", "select": "_id name" } ).populate( { "path": "_creator", "select": "_id name" } ).populate( { "path": "_editor", "select": "_id name" } ).lean();
    } else if ( Object.entries( req.query ).length === 0 && req.query.constructor === Object ) {
      dataResponse = await Agency.find( {} ).populate( { "path": "_account", "select": "_id name" } ).populate( { "path": "_creator", "select": "_id name" } ).populate( { "path": "_editor", "select": "_id name" } ).lean();
    }
